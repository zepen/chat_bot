...
(function ($) {
    ...
    $(window).on('beforeunload', function (e) {
        if (!saved) {
            return '你的文本还没有保存，确定要退出吗？';
        }
    });

	$(window).on('resize', function () {
		var height = $(window).height() - $('.runner-toolbar').height() - $('#hdr').height();
		$('.runner-panel').height(height);
	}).trigger('resize');

	if ($(window).width() < 760) {
		$('.runner-console').addClass('ui-layout-south');
		$('.runner-panel').layout({
			south__size: '50%',
			south__minSize: '33%',
			south__maxSize: '66%',
		...
		$('.runner-console').addClass('ui-layout-east');
		$('.runner-panel').layout({
			east__size: '50%',
			east__minSize: '33%',
			east__maxSize: '66%',
	...
	CodeMirror.modeURL = "//s4.tool.lu/js/codemirror/mode/%N/%N.js";
	...
	function setLang(cm) {
		var mode = langMode[$('#language').val()];
	...
	var templates = {
		...
		'js': 'console.log("hello https://tool.lu/");',
		'lua': 'print(string.find("hello https://tool.lu/", "(tool.lu)"))',
		'groovy': 'println("hello https://tool.lu/")',
		'bash': 'switch=true\nif [[ "$switch" == true ]]\nthen\n\techo "hello https://tool.lu/"\nelse\n\techo "hi https://tool.lu/"\nfi'
	};
	function setDefault(cm, language) {
		var val = cm.getValue();
		silent = true;
		if ((val == '' || changed === false) && !storeId) {
			cm.setValue(templates[language] || '');
		}
		silent = false;
	}
	...
	// 	$('#console').empty();
	// 	// output = [];
	// 	$.post('http://sandbox.tool.lu/run/' + id, {
	// 		code: cm.getValue(),
	// 		language: language
	// 	}, function (data) {
	// 		if ("error" in data) {
 ...
 //        saved = false;
	...
	// 		} else {
	// 			storeId = data.content;
	// 			if (window.history.pushState) {
	// 				window.history.pushState({}, null, '/coderunner/?id=' + storeId);
	// 			} else {
	// 				window.location.href = 'https://tool.lu/coderunner/?id=' + storeId;
	// 			}
	// 			// var embedCode = '<iframe src="https://tool.lu/coderunner/embed.html?id=' + storeId + '" width="650" height="550" frameborder="0"></iframe>';
	// 			var embedCode = '&lt;iframe src="https://tool.lu/coderunner/embed/' + storeId + '.html" width="650" height="550" frameborder="0" mozallowfullscreen webkitallowfullscreen allowfullscreen&gt;&lt;/iframe&gt;';
	// 			$.facebox('拷贝下面的代码，并粘贴到你的博客：<textarea style="display: block; padding: 3px 5px; width: 365px; height: 150px; margin-top: 10px; background-color: #EEE; border: none;">' + embedCode + '</textarea>');
	// 		}
 ...
 //                saved = true;
	...
	editor.setOption("theme", "base16-ocean-dark");

	$('.btn-reindent').on('click', function (e) {
		e.preventDefault();
		editor.reindent();
	});
	...
	$('.btn-indent').on('click', function (e) {
		e.preventDefault();
		editor.execCommand('indentMore');
	});

	$('.btn-unindent').on('click', function (e) {
		e.preventDefault();
		editor.execCommand('indentLess');
	});

	$('#console_clear').on('click', function (e) {
		e.preventDefault();
		$('#console').empty();
	});
	...
	// 		evtSource.close();
	// 	}
	// 	evtSource = new EventSource("http://sandbox.tool.lu/tail/" + id);
	// 	evtSource.onopen = function (e) {
	// 		$('#console .welcome').html('sandbox&gt; tool.lu connected<span class="dotting"></span>');
	// 	};
	// 	evtSource.onmessage = function(e) {
	// 		var html = ansi_up.ansi_to_html($('<div/>').text(e.data).html());
	// 		$('#console').append($('<div/>').attr('class', 'console-line').html(html));
	// 	};
